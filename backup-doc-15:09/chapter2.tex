...
\begin{itemize}
	...
	\item Signals are bent due to sound speed variation along the water column and occluded in many different surfaces, which may lead to the incorrect detection of the line-of-sight (LOS) signal;
...
It is necessary to take into consideration these conditions in order to achieve an effective communication system that uses the underwater acoustic channel. Additionally, when developing a system, it is not always possible to tackle all these constraints, so different priorities should be defined depending on the operation circumstances and application purpose. 

In this section, a detailed overview is presented on various concepts that affect the underwater communication channels, such as the sound speed, multipath phenomena, the Doppler effect, signal attenuation and signal-to-noise ratio.
...
Multipath occurs when signals suffer distortion which originate multiple propagation paths, leading to a change in their original characteristics. This phenomenon is originated by diverse factors that cause distortion in the underwater channels, typically affecting the water composition, such as water temperature and depth. The signal distortions caused by multipath include signal fading, which is usually modeled by Rayleigh fading channel theory \cite{multipath-rayleigh-models}, varying operation frequency, time-variant propagation delays, among others.
...
\paragraph{Shallow water paths} In shallow water, the acoustic signals can be reflected or refracted on the surface, where attenuation is generally smaller then at the bottom of the ocean, where it suffers a higher attenuation depending on the soil material, frequency and incidence angle. Figure \ref{fig:mpath} represent a typical multipath caused by reflection on the sea surface and bottom. 
...
\begin{itemize}
	...
	\item \textit{Surface duct}: the signal gets confined in a surface layer, where the sound velocity increases with depth, due to varied thermal conditions in deeper layers, which causes it to reflect on the surface and bounce back when reaching the subsequent layer.
	...
	\item \textit{Deep sound channel}: it is originated in levels which are surrounded by layers with characteristics highly dependent on depth. So, the signal is constantly bent according to the depth that leads to a minimum speed. 
...
When considering underwater communication systems, it is essential to quantify the attenuation of the channel, i.e. the part of the signal's energy that is absorbed by the involving surrounding. In underwater channels, this absorbance is frequency variable and it is also dependent on physical characteristics of the water, as salinity and temperature. 
...
Signals that are heavily attenuated can become an issue when dealing with acoustic positioning systems, such as USBL systems. If the transmitted signals suffer high attenuation then it can be unfeasible to rigorously detect the signal in the receiver, therefore compute the range or angle of arrival of the transmitter.
...
\begin{table}[!htbp] %use !htbp to adjust
	...
	\begin{tabular}{l|l l l l}
		\toprule
		\textbf{Company} & \textbf{System} & \textbf{Bandwidth(kHz)} &\textbf{Connection(kbps)} & \textbf{Range(m)} \\ 
		\midrule
...
\note{PAPER: Optimal-receiver-config-short-baseline-localization-systems-using-particle-swarm-opt }
